...
static void vector_addition(T *bufferC, T *bufferB, T *bufferA, unsigned int l_size, int alpha, int beta) {
    for (unsigned int i = 0; i < l_size; i++){
        bufferC[i] = bufferA[i] * alpha + bufferB[i] * beta;
    }
}
...
int main_kernel1() {
    ...
    for(unsigned int byte_index = base_tasklet; byte_index < input_size_dpu_bytes; byte_index += BLOCK_SIZE * NR_TASKLETS){
        ...
        vector_addition(cache_C, cache_B, cache_A, l_size_bytes >> DIV, alpha, beta);
