...
int main() {
  ...
  if ((tasklet_id * element_per_cacheC) < rest_rows) rows_per_tasklet += element_per_cacheC;
  if (rest_rows > 0) {
    if ((tasklet_id * element_per_cacheC) >= rest_rows) {
      ...
      // start_row = (hlf_rest_rows + 1) * (dbl_chunks + 2) + (tasklet_id - 1 - hlf_rest_rows) *
      // dbl_chunks;
      else
  ...
  uint32_t mram_base_addr_C = (uint32_t)(DPU_MRAM_HEAP_POINTER + max_rows * n_size_pad * sizeof(T) +
  ...
  for (unsigned int i = start_row; i < start_row + rows_per_tasklet; i += element_per_cacheC) {
    ...
    // for(unsigned int pos = 0; (pos < element_per_cacheC) && ((i + pos) < (start_row +
    // rows_per_tasklet)); pos++){ for(unsigned int pos = 0; pos < element_per_cacheC && i + pos <
    // nr_rows; pos++){
