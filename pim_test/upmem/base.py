    ...
    def h2d(self):
        for symbol in self.symbols:
            if symbol != self.output_symbol:
                self.dev.__setattr__(symbol, 
                    tvm.nd.array(self.host.__getattr__(symbol), self.target_device, symbol=self.func[f"copy_{symbol}"]))
    ...
    def test(self, scheduler, **kwargs):
        ...
        try:
            with open("./results/" + fname + ".txt", "w") as f:
                ...
                self.post_kernel(f)
                flag = self.is_passed()
                print("{:<30} {:<8.3f} {:<8.3f} {:<8.3f} {:<6} {:<10}".format
                  (fname, kernel_time, after_kernel_time, total_time, "PASS" if flag else "FAIL", kwargs.__repr__()))
