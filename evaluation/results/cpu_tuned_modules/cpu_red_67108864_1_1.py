...
@I.ir_module
class module_cpu_red_67108864_1_1:
    @T.prim_func
    def main(A: T.Buffer((67108864,), "int64"), B: T.Buffer((1,), "int64")):
        ...
        # with T.block("root"):
        B_rf = T.alloc_buffer((1048576, 1), "int64")
        for i_0_fused in T.parallel(1048576, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(1)}):
            with T.block("C_rf_init"):
                vi_0 = T.axis.spatial(1048576, i_0_fused)
                T.reads()
        ...
        with T.block("C_init"):
            ...
            B[0] = T.int64(0)
        for i_0 in T.serial(1048576, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(1)}):
            with T.block("C_update"):
                vi_0 = T.axis.reduce(1048576, i_0)
                T.reads(B[0], B_rf[vi_0, 0])
                T.writes(B[0])
                T.block_attr({"meta_schedule.random_compute_producer": T.int64(1)})
                B[0] = B[0] + B_rf[vi_0, 0]
# from tvm import tir
def apply_trace_cpu_red_67108864_1_1(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, = sch.get_loops(block=b0)
  v3, v4 = sch.sample_perfect_tile(loop=l2, n=2, max_innermost_factor=64, decision=[1048576, 64])
