...
@I.ir_module
class module_cpu_mmtv_256_128_256:
    @T.prim_func
    def main(A: T.Buffer((256, 128, 256), "int32"), B: T.Buffer((256, 256), "int32"), C: T.Buffer((256, 128), "int32")):
        ...
        # with T.block("root"):
        for i_0_j_0_i_1_j_1_fused in T.parallel(2048, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(1)}):
            for i_2_init, j_2_init, i_3_init, j_3_init in T.grid(1, 8, 2, 1):
                with T.block("C_init"):
                    v_i = T.axis.spatial(256, i_0_j_0_i_1_j_1_fused // 32 * 4 + i_0_j_0_i_1_j_1_fused % 8 // 4 * 2 + i_2_init * 2 + i_3_init)
                    v_j = T.axis.spatial(128, i_0_j_0_i_1_j_1_fused % 32 // 8 * 32 + i_0_j_0_i_1_j_1_fused % 4 * 8 + j_2_init + j_3_init)
                    T.reads()
                    T.writes(C[v_i, v_j])
                    T.block_attr({"meta_schedule.tiling_structure": "SSRSRS"})
                    C[v_i, v_j] = 0
            for k_0, i_2, j_2, k_1, i_3, j_3 in T.grid(256, 1, 8, 1, 2, 1):
                with T.block("C_update"):
                    v_i = T.axis.spatial(256, i_0_j_0_i_1_j_1_fused // 32 * 4 + i_0_j_0_i_1_j_1_fused % 8 // 4 * 2 + i_2 * 2 + i_3)
                    v_j = T.axis.spatial(128, i_0_j_0_i_1_j_1_fused % 32 // 8 * 32 + i_0_j_0_i_1_j_1_fused % 4 * 8 + j_2 + j_3)
                    v_k = T.axis.reduce(256, k_0 + k_1)
                    T.reads(C[v_i, v_j], A[v_i, v_j, v_k], B[v_i, v_k])
                    T.writes(C[v_i, v_j])
                    T.block_attr({"meta_schedule.tiling_structure": "SSRSRS"})
                    C[v_i, v_j] = C[v_i, v_j] + A[v_i, v_j, v_k] * B[v_i, v_k]
# from tvm import tir
def apply_trace_cpu_mmtv_256_128_256(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  sch.annotate(block_or_loop=b0, ann_key="meta_schedule.tiling_structure", ann_val="SSRSRS")
  l2, l3, l4 = sch.get_loops(block=b0)
  v5, v6, v7, v8 = sch.sample_perfect_tile(loop=l2, n=4, max_innermost_factor=64, decision=[64, 2, 1, 2])
  l9, l10, l11, l12 = sch.split(loop=l2, factors=[v5, v6, v7, v8], preserve_unit_iters=True)
  v13, v14, v15, v16 = sch.sample_perfect_tile(loop=l3, n=4, max_innermost_factor=64, decision=[4, 4, 8, 1])
