...
@I.ir_module
class module_cpu_va_16777216_1_1:
    @T.prim_func
    def main(A: T.Buffer((16777216,), "int32"), B: T.Buffer((16777216,), "int32"), C: T.Buffer((16777216,), "int32")):
        ...
        # with T.block("root"):
        for i_fused_0 in T.parallel(262144):
            for i_fused_1 in T.vectorized(64):
                with T.block("C"):
                    v_i = T.axis.spatial(16777216, i_fused_0 * 64 + i_fused_1)
                    T.reads(A[v_i], B[v_i])
                    T.writes(C[v_i])
                    C[v_i] = A[v_i] + B[v_i]
# from tvm import tir
def apply_trace_cpu_va_16777216_1_1(sch: tir.Schedule) -> None:
