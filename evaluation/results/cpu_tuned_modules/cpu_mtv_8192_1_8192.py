...
@I.ir_module
class module_cpu_mtv_8192_1_8192:
    @T.prim_func
    def main(A: T.Buffer((8192, 8192), "int32"), B: T.Buffer((8192,), "int32"), C: T.Buffer((8192,), "int32")):
        ...
        for i_0_fused in T.parallel(8192, annotations={"pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(1)}):
            for i_1 in range(1):
                for i_2_init, i_3_init in T.grid(1, 1):
                    with T.block("C_init"):
                        ...
                        C_global[v_i] = 0
                for k_0, i_2, k_1, i_3 in T.grid(8192, 1, 1, 1):
                    with T.block("C_update"):
                        v_i = T.axis.spatial(8192, i_0_fused + i_1 + i_2 + i_3)
                        v_k = T.axis.reduce(8192, k_0 + k_1)
            ...
            for ax0 in range(1):
                with T.block("C_global"):
                    ...
                    C[v0] = C_global[v0]
# from tvm import tir
def apply_trace_cpu_mtv_8192_1_8192(sch: tir.Schedule) -> None:
  ...
  v12, v13 = sch.sample_perfect_tile(loop=l3, n=2, max_innermost_factor=64, decision=[8192, 1])
  ...
  b37 = sch.decompose_reduction(block=b30, loop=l33)
