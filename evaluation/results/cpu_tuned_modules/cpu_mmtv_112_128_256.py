...
@I.ir_module
class module_cpu_mmtv_112_128_256:
    @T.prim_func
    def main(A: T.Buffer((112, 128, 256), "int32"), B: T.Buffer((112, 256), "int32"), C: T.Buffer((112, 128), "int32")):
        ...
        for i_0_j_0_i_1_j_1_fused_fused in T.parallel(896):
            for i_2_init, j_2_init, i_3_init in T.grid(2, 4, 1):
                for j_3_fused_init in T.vectorized(2):
                    with T.block("C_init"):
                        v_i = T.axis.spatial(112, i_0_j_0_i_1_j_1_fused_fused // 128 * 16 + i_0_j_0_i_1_j_1_fused_fused % 32 // 4 * 2 + i_2_init + i_3_init)
                        v_j = T.axis.spatial(128, i_0_j_0_i_1_j_1_fused_fused % 128 // 32 * 32 + i_0_j_0_i_1_j_1_fused_fused % 4 * 8 + j_2_init * 2 + j_3_fused_init)
                        T.reads()
                        T.writes(C_global[v_i, v_j])
                        T.block_attr({"meta_schedule.tiling_structure": "SSRSRS"})
                        C_global[v_i, v_j] = 0
            for k_0, i_2, j_2, k_1, i_3 in T.grid(128, 2, 4, 2, 1):
                for j_3_fused in T.vectorized(2):
                    with T.block("C_update"):
                        v_i = T.axis.spatial(112, i_0_j_0_i_1_j_1_fused_fused // 128 * 16 + i_0_j_0_i_1_j_1_fused_fused % 32 // 4 * 2 + i_2 + i_3)
                        v_j = T.axis.spatial(128, i_0_j_0_i_1_j_1_fused_fused % 128 // 32 * 32 + i_0_j_0_i_1_j_1_fused_fused % 4 * 8 + j_2 * 2 + j_3_fused)
            ...
            for ax0 in range(2):
                for ax1_fused in T.vectorized(8):
                    with T.block("C_global"):
                        v0 = T.axis.spatial(112, i_0_j_0_i_1_j_1_fused_fused // 128 * 16 + i_0_j_0_i_1_j_1_fused_fused % 32 // 4 * 2 + ax0)
                        v1 = T.axis.spatial(128, i_0_j_0_i_1_j_1_fused_fused % 128 // 32 * 32 + i_0_j_0_i_1_j_1_fused_fused % 4 * 8 + ax1_fused)
                        T.reads(C_global[v0, v1])
                        T.writes(C[v0, v1])
                        C[v0, v1] = C_global[v0, v1]
# from tvm import tir
def apply_trace_cpu_mmtv_112_128_256(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  sch.annotate(block_or_loop=b0, ann_key="meta_schedule.tiling_structure", ann_val="SSRSRS")
  l2, l3, l4 = sch.get_loops(block=b0)
  v5, v6, v7, v8 = sch.sample_perfect_tile(loop=l2, n=4, max_innermost_factor=64, decision=[7, 8, 2, 1])
  l9, l10, l11, l12 = sch.split(loop=l2, factors=[v5, v6, v7, v8], preserve_unit_iters=True)
  v13, v14, v15, v16 = sch.sample_perfect_tile(loop=l3, n=4, max_innermost_factor=64, decision=[4, 4, 4, 2])
