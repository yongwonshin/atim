...
@I.ir_module
class module_cpu_gemv_4096_1_4096:
    @T.prim_func
    def main(A: T.Buffer((4096, 4096), "int32"), B: T.Buffer((4096,), "int32"), C: T.Buffer((4096,), "int32"), ALPHA: T.Buffer((1,), "int32")):
        ...
        for i_0_fused in T.parallel(512, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(1)}):
            for i_1 in range(1):
                for i_2_init, i_3_init in T.grid(8, 1):
                    with T.block("C_init"):
                        v_i = T.axis.spatial(4096, i_0_fused * 8 + i_1 * 8 + i_2_init + i_3_init)
                        T.reads()
                        T.writes(C[v_i])
                ...
                for k_0, i_2, k_1, i_3 in T.grid(4096, 8, 1, 1):
                    with T.block("C_update"):
                        v_i = T.axis.spatial(4096, i_0_fused * 8 + i_1 * 8 + i_2 + i_3)
...
# from tvm import tir
def apply_trace_cpu_gemv_4096_1_4096(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  sch.annotate(block_or_loop=b0, ann_key="meta_schedule.tiling_structure", ann_val="SSRSRS")
  l2, l3 = sch.get_loops(block=b0)
  v4, v5, v6, v7 = sch.sample_perfect_tile(loop=l2, n=4, max_innermost_factor=64, decision=[512, 1, 8, 1])
