...
@I.ir_module
class module_cpu_geva_67108864_1_1:
    @T.prim_func
    def main(A: T.Buffer((67108864,), "int32"), B: T.Buffer((67108864,), "int32"), C: T.Buffer((67108864,), "int32"), ALPHA: T.Buffer((1,), "int32"), BETA: T.Buffer((1,), "int32")):
        ...
        for i_fused_0 in T.parallel(1048576):
            for i_fused_1 in T.vectorized(64):
                with T.block("C"):
                    v_i = T.axis.spatial(67108864, i_fused_0 * 64 + i_fused_1)
                    T.reads(A[v_i], B[v_i])
                    T.writes(C[v_i])
                    C[v_i] = alpha_val * A[v_i] + beta_val * B[v_i]
# from tvm import tir
def apply_trace_cpu_geva_67108864_1_1(sch: tir.Schedule) -> None:
