...
@I.ir_module
class module_cpu_mmtv_64_64_256:
    @T.prim_func
        ...
        for i_0_j_0_i_1_j_1_fused in T.parallel(512, annotations={"pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(1)}):
            for i_2_init, j_2_init, i_3_init in T.grid(1, 4, 1):
                for j_3_fused_init in T.vectorized(2):
                    with T.block("C_init"):
                        v_i = T.axis.spatial(64, i_0_j_0_i_1_j_1_fused // 16 * 2 + i_0_j_0_i_1_j_1_fused % 4 // 2 + i_2_init + i_3_init)
                        v_j = T.axis.spatial(64, i_0_j_0_i_1_j_1_fused % 16 // 4 * 16 + i_0_j_0_i_1_j_1_fused % 2 * 8 + j_2_init * 2 + j_3_fused_init)
                        T.reads()
            ...
            for k_0, i_2, j_2, k_1, i_3 in T.grid(64, 1, 4, 4, 1):
                for j_3_fused in T.vectorized(2):
                    with T.block("C_update"):
                        v_i = T.axis.spatial(64, i_0_j_0_i_1_j_1_fused // 16 * 2 + i_0_j_0_i_1_j_1_fused % 4 // 2 + i_2 + i_3)
                        v_j = T.axis.spatial(64, i_0_j_0_i_1_j_1_fused % 16 // 4 * 16 + i_0_j_0_i_1_j_1_fused % 2 * 8 + j_2 * 2 + j_3_fused)
            ...
            for ax0 in range(1):
                for ax1_fused in T.vectorized(8):
                    with T.block("C_global"):
                        v0 = T.axis.spatial(64, i_0_j_0_i_1_j_1_fused // 16 * 2 + i_0_j_0_i_1_j_1_fused % 4 // 2 + ax0)
                        v1 = T.axis.spatial(64, i_0_j_0_i_1_j_1_fused % 16 // 4 * 16 + i_0_j_0_i_1_j_1_fused % 2 * 8 + ax1_fused)
                        T.reads(C_global[v0, v1])
                        T.writes(C[v0, v1])
                        C[v0, v1] = C_global[v0, v1]
# from tvm import tir
def apply_trace_cpu_mmtv_64_64_256(sch: tir.Schedule) -> None:
