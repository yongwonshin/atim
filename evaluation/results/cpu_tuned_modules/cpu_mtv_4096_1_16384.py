...
@I.ir_module
class module_cpu_mtv_4096_1_16384:
    @T.prim_func
    def main(A: T.Buffer((4096, 16384), "int32"), B: T.Buffer((16384,), "int32"), C: T.Buffer((4096,), "int32")):
        ...
        # with T.block("root"):
        C_global = T.alloc_buffer((4096,), "int32")
        for i_0_fused in T.parallel(4096, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(1)}):
            for i_1 in range(1):
                for i_2_init, i_3_init in T.grid(1, 1):
                    with T.block("C_init"):
                        v_i = T.axis.spatial(4096, i_0_fused + i_1 + i_2_init + i_3_init)
                        T.reads()
                        T.writes(C_global[v_i])
                        T.block_attr({"meta_schedule.tiling_structure": "SSRSRS"})
                        C_global[v_i] = 0
                for k_0, i_2, k_1, i_3 in T.grid(512, 1, 32, 1):
                    with T.block("C_update"):
                        v_i = T.axis.spatial(4096, i_0_fused + i_1 + i_2 + i_3)
                        v_k = T.axis.reduce(16384, k_0 * 32 + k_1)
            ...
            for ax0 in range(1):
                with T.block("C_global"):
                    v0 = T.axis.spatial(4096, i_0_fused + ax0)
                    T.reads(C_global[v0])
                    T.writes(C[v0])
                    C[v0] = C_global[v0]
# from tvm import tir
def apply_trace_cpu_mtv_4096_1_16384(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  sch.annotate(block_or_loop=b0, ann_key="meta_schedule.tiling_structure", ann_val="SSRSRS")
  l2, l3 = sch.get_loops(block=b0)
  v4, v5, v6, v7 = sch.sample_perfect_tile(loop=l2, n=4, max_innermost_factor=64, decision=[4096, 1, 1, 1])
  l8, l9, l10, l11 = sch.split(loop=l2, factors=[v4, v5, v6, v7], preserve_unit_iters=True)
  v12, v13 = sch.sample_perfect_tile(loop=l3, n=2, max_innermost_factor=64, decision=[512, 32])
