...
@I.ir_module
class module_cpu_ttv_256_512_512:
    @T.prim_func
    def main(A: T.Buffer((256, 512, 512), "int32"), B: T.Buffer((512,), "int32"), C: T.Buffer((256, 512), "int32")):
        ...
        for i_0_j_0_fused in T.parallel(8192, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(1)}):
            for i_1, j_1 in T.grid(1, 8):
                for i_2_init, j_2_init, i_3_init in T.grid(1, 1, 1):
                    for j_3_fused_init in T.vectorized(2):
                        with T.block("C_init"):
                            v_i = T.axis.spatial(256, i_0_j_0_fused // 32 + i_1 + i_2_init + i_3_init)
                            v_j = T.axis.spatial(512, i_0_j_0_fused % 32 * 16 + j_1 * 2 + j_2_init * 2 + j_3_fused_init)
                            T.reads()
                ...
                for k_0, i_2, j_2, k_1, i_3 in T.grid(8, 1, 1, 64, 1):
                    for j_3_fused in T.vectorized(2):
                        with T.block("C_update"):
                            v_i = T.axis.spatial(256, i_0_j_0_fused // 32 + i_1 + i_2 + i_3)
                            v_j = T.axis.spatial(512, i_0_j_0_fused % 32 * 16 + j_1 * 2 + j_2 * 2 + j_3_fused)
                ...
                for ax0 in range(1):
                    for ax1_fused in T.vectorized(2):
                        with T.block("C_global"):
                            ...
                            v1 = T.axis.spatial(512, i_0_j_0_fused % 32 * 16 + j_1 * 2 + ax1_fused)
                            T.reads(C_global[v0, v1])
                            T.writes(C[v0, v1])
                            C[v0, v1] = C_global[v0, v1]
# from tvm import tir
def apply_trace_cpu_ttv_256_512_512(sch: tir.Schedule) -> None:
  ...
  v13, v14, v15, v16 = sch.sample_perfect_tile(loop=l3, n=4, max_innermost_factor=64, decision=[32, 8, 1, 2])
