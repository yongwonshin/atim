...
@I.ir_module
class module_atim_mtv_21504_1_7168:
    @T.prim_func
    def main(A: T.Buffer((21504, 7168), "int32"), B: T.Buffer((7168,), "int32"), C: T.Buffer((21504,), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((32, 21504), "int32")
            C_rf_global_local = T.alloc_buffer((32, 21504), "int32", scope="local")
            ...
            for k_0 in T.thread_binding(32, thread="blockIdx.x", annotations={"bank": T.int64(1)}):
                for i_0 in T.thread_binding(64, thread="blockIdx.y", annotations={"bank": T.int64(1)}):
                    for i_1 in T.thread_binding(21, thread="threadIdx.x"):
                        for i_2 in range(4):
                            for i_3_init, i_4_init in T.grid(4, 1):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(21504, i_0 * 336 + i_1 * 16 + i_2 * 4 + i_3_init + i_4_init)
                            ...
                            for k_1_0 in range(7):
                                for ax0, ax1 in T.grid(4, 32):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(21504, i_0 * 336 + i_1 * 16 + i_2 * 4 + ax0)
                                        v1 = T.axis.spatial(7168, k_0 * 224 + k_1_0 * 32 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(32):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(7168, k_0 * 224 + k_1_0 * 32 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_3 in range(4):
                                    for k_1_1, i_4, k_1_2 in T.grid(2, 1, 16):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(21504, i_0 * 336 + i_1 * 16 + i_2 * 4 + i_3 + i_4)
                                            vk_0 = T.axis.spatial(32, k_0)
                                            vk_1 = T.axis.reduce(224, k_1_0 * 32 + k_1_1 * 16 + k_1_2)
...
# from tvm import tir
def apply_trace_atim_mtv_21504_1_7168(sch: tir.Schedule) -> None:
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[64, 21, 4, 4, 1])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[7, 2, 16])
