...
@I.ir_module
class module_gptj_mmtv_64_128_256:
    @T.prim_func
    def main(A: T.Buffer((64, 128, 256), "int32"), B: T.Buffer((64, 256), "int32"), C: T.Buffer((64, 128), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((64, 4, 128), "int32")
            C_rf_global_local = T.alloc_buffer((64, 4, 128), "int32", scope="local")
            A_local = T.alloc_buffer((64, 128, 256), "int32", scope="local")
                            ...
                            for i_1, i_2, j_2 in T.grid(1, 1, 1):
                                for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 2, 1, 2):
                                    with T.block("C_rf_init"):
                                        ...
                                        v_j = T.axis.spatial(128, j_0 * 64 + j_1 * 4 + j_2 * 4 + j_3_init * 2 + j_4_init)
                                ...
                                for k_1_0 in range(8):
                                    for ax0, ax1, ax2 in T.grid(1, 4, 8):
                                        with T.block("A_local"):
                                            v0 = T.axis.spatial(64, i_0 + ax0)
                                            v1 = T.axis.spatial(128, j_0 * 64 + j_1 * 4 + ax1)
                                    ...
                                    for i_3, j_3, k_1_1, i_4, j_4, k_1_2 in T.grid(1, 2, 1, 1, 2, 8):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(64, i_0 + i_1 + i_2 + i_3 + i_4)
                                            v_j = T.axis.spatial(128, j_0 * 64 + j_1 * 4 + j_2 * 4 + j_3 * 2 + j_4)
                                            vk_0 = T.axis.spatial(4, k_0)
                                ...
                                for ax0, ax1, ax2 in T.grid(1, 1, 4):
                                    with T.block("C_rf_global_local"):
                                        ...
                                        v2 = T.axis.spatial(128, j_0 * 64 + j_1 * 4 + ax2)
...
# from tvm import tir
def apply_trace_gptj_mmtv_64_128_256(sch: tir.Schedule) -> None:
  ...
  v25, v26, v27, v28, v29 = sch.sample_perfect_tile(loop=l12, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 16, 1, 2, 2])
