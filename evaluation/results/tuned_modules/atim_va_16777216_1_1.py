...
@I.ir_module
class module_atim_va_16777216_1_1:
    @T.prim_func
    def main(A: T.Buffer((16777216,), "int32"), B: T.Buffer((16777216,), "int32"), C: T.Buffer((16777216,), "int32")):
        ...
        with T.block("root"):
            ...
            for i_0 in T.thread_binding(2048, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                for i_1 in T.thread_binding(8, thread="threadIdx.x"):
                    for i_2, i_3 in T.grid(2, 2):
                        for ax0 in range(256):
                            with T.block("A_local"):
                                v0 = T.axis.spatial(16777216, i_0 * 8192 + i_1 * 1024 + i_2 * 512 + i_3 * 256 + ax0)
                                T.reads(A[v0])
                                T.writes(A_local[v0])
                        ...
                        for i_4 in range(256):
                            with T.block("C"):
                                v_i = T.axis.spatial(16777216, i_0 * 8192 + i_1 * 1024 + i_2 * 512 + i_3 * 256 + i_4)
                        ...
                        for ax0 in range(256):
                            with T.block("C_local"):
                                ...
                                C[v0] = C_local[v0]
# from tvm import tir
def apply_trace_atim_va_16777216_1_1(sch: tir.Schedule) -> None:
  ...
  v3, v4, v5, v6, v7 = sch.sample_perfect_tile(loop=l2, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[2048, 8, 2, 2, 256])
  ...
  sch.reverse_compute_at(block=b13, loop=l11, preserve_unit_loops=True, index=-1)
  ...
  l40, l41, l42, l43, l44 = sch.get_loops(block=b24)
