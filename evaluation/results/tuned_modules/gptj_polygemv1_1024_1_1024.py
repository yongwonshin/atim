...
@I.ir_module
class module_gptj_polygemv1_1024_1_1024:
    @T.prim_func
                        ...
                        for i_2 in range(2):
                            for i_3_init, i_4_init in T.grid(1, 2):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(1024, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3_init * 2 + i_4_init)
                            ...
                            for k_1_0 in range(2):
                                for ax0, ax1 in T.grid(2, 32):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(1024, i_0 * 64 + i_1 * 4 + i_2 * 2 + ax0)
                                        v1 = T.axis.spatial(1024, k_0 * 64 + k_1_0 * 32 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(32):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(1024, k_0 * 64 + k_1_0 * 32 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_3, k_1_1, i_4, k_1_2 in T.grid(1, 1, 2, 32):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(1024, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3 * 2 + i_4)
                                        vk_0 = T.axis.spatial(16, k_0)
                                        vk_1 = T.axis.reduce(64, k_1_0 * 32 + k_1_1 * 32 + k_1_2)
                                        T.reads(C_rf_global_local[vk_0, v_i], A_local[v_i, vk_0 * 64 + vk_1], B_local[vk_0 * 64 + vk_1])
                                        T.writes(C_rf_global_local[vk_0, v_i])
                                        T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SSSRSRSR"})
                                        C_rf_global_local[vk_0, v_i] = C_rf_global_local[vk_0, v_i] + alpha_val * A_local[v_i, vk_0 * 64 + vk_1] * B_local[vk_0 * 64 + vk_1]
                            for ax0, ax1 in T.grid(1, 2):
                                with T.block("C_rf_global_local"):
                                    v0 = T.axis.spatial(16, k_0 + ax0)
                                    v1 = T.axis.spatial(1024, i_0 * 64 + i_1 * 4 + i_2 * 2 + ax1)
...
# from tvm import tir
def apply_trace_gptj_polygemv1_1024_1_1024(sch: tir.Schedule) -> None:
  ...
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 1, 32])
