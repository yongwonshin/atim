...
@I.ir_module
class module_gptj_mtv_8192_1_16384:
    @T.prim_func
    def main(A: T.Buffer((8192, 16384), "int32"), B: T.Buffer((16384,), "int32"), C: T.Buffer((8192,), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((64, 8192), "int32")
            C_rf_global_local = T.alloc_buffer((64, 8192), "int32", scope="local")
            A_local = T.alloc_buffer((8192, 16384), "int32", scope="local")
            B_local = T.alloc_buffer((16384,), "int32", scope="local")
            for k_0 in T.thread_binding(64, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                        ...
                        for i_2 in range(8):
                            for i_3_init, i_4_init in T.grid(1, 2):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(8192, i_0 * 256 + i_1 * 16 + i_2 * 2 + i_3_init * 2 + i_4_init)
                            ...
                            for k_1_0, i_3 in T.grid(8, 1):
                                for k_1_1 in range(1):
                                    for ax0, ax1 in T.grid(2, 32):
                                        with T.block("A_local"):
                                            v0 = T.axis.spatial(8192, i_0 * 256 + i_1 * 16 + i_2 * 2 + ax0)
                                            v1 = T.axis.spatial(16384, k_0 * 256 + k_1_0 * 32 + ax1)
                                            T.reads(A[v0, v1])
                                    ...
                                    for ax0 in range(32):
                                        with T.block("B_local"):
                                            v0 = T.axis.spatial(16384, k_0 * 256 + k_1_0 * 32 + ax0)
                                            T.reads(B[v0])
                                            T.writes(B_local[v0])
                                            T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                            B_local[v0] = B[v0]
                                    for i_4, k_1_2 in T.grid(2, 32):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(8192, i_0 * 256 + i_1 * 16 + i_2 * 2 + i_3 * 2 + i_4)
                                            vk_0 = T.axis.spatial(64, k_0)
                                            vk_1 = T.axis.reduce(256, k_1_0 * 32 + k_1_1 * 32 + k_1_2)
                                ...
                                for ax0, ax1 in T.grid(1, 2):
                                    with T.block("C_rf_global_local"):
                                        ...
                                        v1 = T.axis.spatial(8192, i_0 * 256 + i_1 * 16 + i_2 * 2 + ax1)
                                        T.reads(C_rf_global_local[v0, v1])
                                        T.writes(C_rf_global[v0, v1])
                                        C_rf_global[v0, v1] = C_rf_global_local[v0, v1]
            for i in T.serial(8192, annotations={"pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                with T.block("C_init"):
...
# from tvm import tir
def apply_trace_gptj_mtv_8192_1_16384(sch: tir.Schedule) -> None:
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[32, 16, 8, 1, 2])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[8, 1, 32])
  ...
  sch.annotate(block_or_loop=l51, ann_key="pragma_auto_unroll_max_step", ann_val=64)
  sch.annotate(block_or_loop=l51, ann_key="pragma_unroll_explicit", ann_val=0)
  l59, l60, l61, l62, l63, l64, l65, l66, l67 = sch.get_loops(block=b39)
  sch.annotate(block_or_loop=l59, ann_key="pragma_auto_unroll_max_step", ann_val=64)
  ...
  sch.annotate(block_or_loop=l76, ann_key="pragma_unroll_explicit", ann_val=0)
