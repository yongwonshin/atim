...
@I.ir_module
class module_atim_mtv_12288_1_4096:
    @T.prim_func
    def main(A: T.Buffer((12288, 4096), "int32"), B: T.Buffer((4096,), "int32"), C: T.Buffer((12288,), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((32, 12288), "int32")
            C_rf_global_local = T.alloc_buffer((32, 12288), "int32", scope="local")
            A_local = T.alloc_buffer((12288, 4096), "int32", scope="local")
            B_local = T.alloc_buffer((4096,), "int32", scope="local")
            for k_0 in T.thread_binding(32, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                for i_0 in T.thread_binding(64, thread="blockIdx.y", annotations={"bank": T.int64(1)}):
                    for i_1 in T.thread_binding(24, thread="threadIdx.x"):
                        for i_2 in range(1):
                            for i_3_init, i_4_init in T.grid(4, 2):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(12288, i_0 * 192 + i_1 * 8 + i_2 * 8 + i_3_init * 2 + i_4_init)
                            ...
                            for k_1_0 in range(16):
                                for ax0, ax1 in T.grid(8, 8):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(12288, i_0 * 192 + i_1 * 8 + ax0)
                                        v1 = T.axis.spatial(4096, k_0 * 128 + k_1_0 * 8 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(8):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(4096, k_0 * 128 + k_1_0 * 8 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_3, k_1_1, i_4, k_1_2 in T.grid(4, 4, 2, 2):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(12288, i_0 * 192 + i_1 * 8 + i_2 * 8 + i_3 * 2 + i_4)
                                        vk_0 = T.axis.spatial(32, k_0)
                                        vk_1 = T.axis.reduce(128, k_1_0 * 8 + k_1_1 * 2 + k_1_2)
                            ...
                            for ax0, ax1 in T.grid(1, 8):
                                with T.block("C_rf_global_local"):
                                    ...
                                    v1 = T.axis.spatial(12288, i_0 * 192 + i_1 * 8 + ax1)
                                    T.reads(C_rf_global_local[v0, v1])
                                    T.writes(C_rf_global[v0, v1])
                                    C_rf_global[v0, v1] = C_rf_global_local[v0, v1]
            for i in T.serial(12288, annotations={"pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                with T.block("C_init"):
...
# from tvm import tir
def apply_trace_atim_mtv_12288_1_4096(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, l3 = sch.get_loops(block=b0)
  v4, v5 = sch.sample_perfect_tile2(loop=l3, n=2, min_n_splits=2, max_n_splits=256, decision=[32, 128])
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[64, 24, 1, 4, 2])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[16, 4, 2])
