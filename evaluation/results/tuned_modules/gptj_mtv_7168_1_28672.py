...
@I.ir_module
class module_gptj_mtv_7168_1_28672:
    @T.prim_func
    def main(A: T.Buffer((7168, 28672), "int32"), B: T.Buffer((28672,), "int32"), C: T.Buffer((7168,), "int32")):
                        ...
                        for i_2 in range(4):
                            for i_3_init, i_4_init in T.grid(1, 2):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(7168, i_0 * 112 + i_1 * 8 + i_2 * 2 + i_3_init * 2 + i_4_init)
                            ...
                            for k_1_0, i_3, k_1_1 in T.grid(1, 1, 64):
                                for ax0, ax1 in T.grid(2, 14):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(7168, i_0 * 112 + i_1 * 8 + i_2 * 2 + ax0)
                                        v1 = T.axis.spatial(28672, k_0 * 896 + k_1_1 * 14 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(14):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(28672, k_0 * 896 + k_1_1 * 14 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_4, k_1_2 in T.grid(2, 14):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(7168, i_0 * 112 + i_1 * 8 + i_2 * 2 + i_3 * 2 + i_4)
                                        vk_0 = T.axis.spatial(32, k_0)
                                        vk_1 = T.axis.reduce(896, k_1_0 * 896 + k_1_1 * 14 + k_1_2)
                                        T.reads(C_rf_global_local[vk_0, v_i], A_local[v_i, vk_0 * 896 + vk_1], B_local[vk_0 * 896 + vk_1])
                                        T.writes(C_rf_global_local[vk_0, v_i])
                                        T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SSSRSRSR"})
                                        C_rf_global_local[vk_0, v_i] = C_rf_global_local[vk_0, v_i] + A_local[v_i, vk_0 * 896 + vk_1] * B_local[vk_0 * 896 + vk_1]
                            for ax0, ax1 in T.grid(1, 2):
                                with T.block("C_rf_global_local"):
                                    v0 = T.axis.spatial(32, k_0 + ax0)
                                    v1 = T.axis.spatial(7168, i_0 * 112 + i_1 * 8 + i_2 * 2 + ax1)
...
# from tvm import tir
def apply_trace_gptj_mtv_7168_1_28672(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, l3 = sch.get_loops(block=b0)
  v4, v5 = sch.sample_perfect_tile2(loop=l3, n=2, min_n_splits=2, max_n_splits=256, decision=[32, 896])
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[64, 14, 4, 1, 2])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[1, 64, 14])
