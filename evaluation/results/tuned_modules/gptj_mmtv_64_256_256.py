...
@I.ir_module
class module_gptj_mmtv_64_256_256:
    @T.prim_func
    def main(A: T.Buffer((64, 256, 256), "int32"), B: T.Buffer((64, 256), "int32"), C: T.Buffer((64, 256), "int32")):
        ...
        with T.block("root"):
            ...
            C_local = T.alloc_buffer((64, 256), "int32", scope="local")
            A_local = T.alloc_buffer((64, 256, 256), "int32", scope="local")
            B_local = T.alloc_buffer((64, 256), "int32", scope="local")
            for i_0 in T.thread_binding(64, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                        ...
                        for i_1, i_2, j_2 in T.grid(1, 1, 1):
                            for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 1, 1, 2):
                                with T.block("C_init"):
                                    ...
                                    v_j = T.axis.spatial(256, j_0 * 16 + j_1 * 2 + j_2 * 2 + j_3_init * 2 + j_4_init)
                                    T.reads()
                                    T.writes(C_local[v_i, v_j])
                                    T.block_attr({"meta_schedule.tiling_structure": "SSSRSRSR"})
                                    C_local[v_i, v_j] = 0
                            for k_0 in range(2):
                                for ax0, ax1, ax2 in T.grid(1, 2, 128):
                                ...
                                for i_3, j_3 in T.grid(1, 1):
                                    for k_1, i_4, j_4, k_2 in T.grid(4, 1, 2, 32):
                                        with T.block("C_update"):
                                            v_i = T.axis.spatial(64, i_0 + i_1 + i_2 + i_3 + i_4)
                                            v_j = T.axis.spatial(256, j_0 * 16 + j_1 * 2 + j_2 * 2 + j_3 * 2 + j_4)
                                            v_k = T.axis.reduce(256, k_0 * 128 + k_1 * 32 + k_2)
...
# from tvm import tir
def apply_trace_gptj_mmtv_64_256_256(sch: tir.Schedule) -> None:
  ...
  v5, v6, v7, v8, v9 = sch.sample_perfect_tile(loop=l2, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[64, 1, 1, 1, 1])
  l10, l11, l12, l13, l14 = sch.split(loop=l2, factors=[v5, v6, v7, v8, v9], preserve_unit_iters=True)
  v15, v16, v17, v18, v19 = sch.sample_perfect_tile(loop=l3, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[16, 8, 1, 1, 2])
  l20, l21, l22, l23, l24 = sch.split(loop=l3, factors=[v15, v16, v17, v18, v19], preserve_unit_iters=True)
  v25, v26, v27 = sch.sample_perfect_tile(loop=l4, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 4, 32])
