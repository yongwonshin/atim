...
@I.ir_module
class module_atim_red_33554432_1_1:
    @T.prim_func
    def main(A: T.Buffer((33554432,), "int64"), B: T.Buffer((1,), "int64")):
        ...
        with T.block("root"):
            ...
            B_rf_global = T.alloc_buffer((1024, 1), "int64")
            B_rf_global_rf_shared = T.alloc_buffer((16, 1024, 1), "int64", scope="shared")
            B_rf_global_rf_shared_local = T.alloc_buffer((16, 1024, 1), "int64", scope="local")
            A_local = T.alloc_buffer((33554432,), "int64", scope="local")
            for i_0 in T.thread_binding(1024, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                for i_1_0 in T.thread_binding(16, thread="threadIdx.x"):
                    ...
                    for i_1_1_0 in range(2):
                        for i_1_1_1 in range(8):
                            for ax0 in range(128):
                                with T.block("A_local"):
                                    v0 = T.axis.spatial(33554432, i_0 * 32768 + i_1_0 * 2048 + i_1_1_0 * 1024 + i_1_1_1 * 128 + ax0)
                                    T.reads(A[v0])
                                    T.writes(A_local[v0])
                                    A_local[v0] = A[v0]
                            for i_1_1_2 in range(128):
                                with T.block("C_rf_rf_update"):
                                    vi_0, vi_1_0 = T.axis.remap("SS", [i_0, i_1_0])
                                    vi_1_1 = T.axis.reduce(2048, i_1_1_0 * 1024 + i_1_1_1 * 128 + i_1_1_2)
                        ...
                        for ax0, ax1, ax2 in T.grid(1, 1, 1):
                            with T.block("B_rf_global_rf_shared_local"):
                                ...
                                v1 = T.axis.spatial(1024, i_0 + ax1)
            ...
            with T.block("C_init"):
                ...
                B[0] = T.int64(0)
            for i_0 in T.serial(1024, annotations={"pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                with T.block("C_update"):
                    vi_0 = T.axis.reduce(1024, i_0)
                    T.reads(B[0], B_rf_global[vi_0, 0])
                    T.writes(B[0])
                    T.block_attr({"meta_schedule.meta_schedule_rfactor_consumer_block": T.int64(1), "meta_schedule.random_compute_producer": T.int64(1)})
                    B[0] = B[0] + B_rf_global[vi_0, 0]
# from tvm import tir
def apply_trace_atim_red_33554432_1_1(sch: tir.Schedule) -> None:
  ...
  v23, v24, v25 = sch.sample_perfect_tile(loop=l22, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 8, 128])
