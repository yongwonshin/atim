...
@I.ir_module
class module_gptj_ttv_512_512_512:
    @T.prim_func
    def main(A: T.Buffer((512, 512, 512), "int32"), B: T.Buffer((512,), "int32"), C: T.Buffer((512, 512), "int32")):
        ...
        with T.block("root"):
            ...
            for i_0 in T.thread_binding(512, thread="blockIdx.x", annotations={"bank": T.int64(1)}):
                        ...
                        for i_1, i_2, j_2 in T.grid(1, 1, 1):
                            for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 1, 1, 8):
                                with T.block("C_init"):
                                    ...
                                    v_j = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 8 + j_3_init * 8 + j_4_init)
                                    T.reads()
                            ...
                            for k_0, i_3, j_3, k_1 in T.grid(32, 1, 1, 1):
                                for ax0, ax1, ax2 in T.grid(1, 8, 16):
                                    with T.block("A_local"):
                                        ...
                                        v1 = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + ax1)
                                ...
                                for ax0 in range(16):
                                    with T.block("B_local"):
                                        ...
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_4, j_4, k_2 in T.grid(1, 8, 16):
                                    with T.block("C_update"):
                                        v_i = T.axis.spatial(512, i_0 + i_1 + i_2 + i_3 + i_4)
                                        v_j = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 8 + j_3 * 8 + j_4)
...
# from tvm import tir
def apply_trace_gptj_ttv_512_512_512(sch: tir.Schedule) -> None:
  ...
  v15, v16, v17, v18, v19 = sch.sample_perfect_tile(loop=l3, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[4, 16, 1, 1, 8])
  ...
  v36 = sch.sample_categorical(candidates=[0, 16, 64, 512], probs=[0.25, 0.25, 0.25, 0.25], decision=0)
