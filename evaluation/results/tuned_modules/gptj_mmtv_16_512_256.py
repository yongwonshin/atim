...
@I.ir_module
class module_gptj_mmtv_16_512_256:
    @T.prim_func
    def main(A: T.Buffer((16, 512, 256), "int32"), B: T.Buffer((16, 256), "int32"), C: T.Buffer((16, 512), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((16, 4, 512), "int32")
            C_rf_global_local = T.alloc_buffer((16, 4, 512), "int32", scope="local")
            A_local = T.alloc_buffer((16, 512, 256), "int32", scope="local")
            B_local = T.alloc_buffer((16, 256), "int32", scope="local")
            for k_0 in T.thread_binding(4, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                            ...
                            for i_1, i_2, j_2 in T.grid(1, 1, 2):
                                for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 1, 1, 2):
                                    with T.block("C_rf_init"):
                                        ...
                                        v_j = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3_init * 2 + j_4_init)
                                ...
                                for k_1_0 in range(2):
                                    for ax0, ax1, ax2 in T.grid(1, 2, 32):
                                        with T.block("A_local"):
                                            ...
                                            v1 = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + ax1)
                                            v2 = T.axis.spatial(256, k_0 * 64 + k_1_0 * 32 + ax2)
                                            T.reads(A[v0, v1, v2])
                                    ...
                                    for i_3, j_3, k_1_1, i_4, j_4, k_1_2 in T.grid(1, 1, 1, 1, 2, 32):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(16, i_0 + i_1 + i_2 + i_3 + i_4)
                                            v_j = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3 * 2 + j_4)
                                            vk_0 = T.axis.spatial(4, k_0)
                                ...
                                for ax0, ax1, ax2 in T.grid(1, 1, 2):
                                    with T.block("C_rf_global_local"):
                                        ...
                                        v2 = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + ax2)
...
# from tvm import tir
def apply_trace_gptj_mmtv_16_512_256(sch: tir.Schedule) -> None:
  ...
  v25, v26, v27, v28, v29 = sch.sample_perfect_tile(loop=l12, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[8, 16, 2, 1, 2])
  l30, l31, l32, l33, l34 = sch.split(loop=l12, factors=[v25, v26, v27, v28, v29], preserve_unit_iters=True)
  v35, v36, v37 = sch.sample_perfect_tile(loop=l14, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 1, 32])
