...
@I.ir_module
class module_gptj_mtv_28672_1_7168:
    @T.prim_func
    def main(A: T.Buffer((28672, 7168), "int32"), B: T.Buffer((7168,), "int32"), C: T.Buffer((28672,), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((32, 28672), "int32")
            C_rf_global_local = T.alloc_buffer((32, 28672), "int32", scope="local")
            ...
            for k_0 in T.thread_binding(32, thread="blockIdx.x", annotations={"bank": T.int64(1)}):
                for i_0 in T.thread_binding(56, thread="blockIdx.y", annotations={"bank": T.int64(1)}):
                    for i_1 in T.thread_binding(16, thread="threadIdx.x"):
                        for i_2 in range(1):
                            for i_3_init, i_4_init in T.grid(16, 2):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(28672, i_0 * 512 + i_1 * 32 + i_2 * 32 + i_3_init * 2 + i_4_init)
                            ...
                            for k_1_0, i_3 in T.grid(2, 16):
                                for k_1_1 in range(7):
                                    for ax0, ax1 in T.grid(2, 16):
                                        with T.block("A_local"):
                                            v0 = T.axis.spatial(28672, i_0 * 512 + i_1 * 32 + i_3 * 2 + ax0)
                                            v1 = T.axis.spatial(7168, k_0 * 224 + k_1_0 * 112 + k_1_1 * 16 + ax1)
                                            T.reads(A[v0, v1])
                                    ...
                                    for ax0 in range(16):
                                        with T.block("B_local"):
                                            v0 = T.axis.spatial(7168, k_0 * 224 + k_1_0 * 112 + k_1_1 * 16 + ax0)
                                            T.reads(B[v0])
                                            T.writes(B_local[v0])
                                            T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                            B_local[v0] = B[v0]
                                    for i_4, k_1_2 in T.grid(2, 16):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(28672, i_0 * 512 + i_1 * 32 + i_2 * 32 + i_3 * 2 + i_4)
                                            vk_0 = T.axis.spatial(32, k_0)
                                            vk_1 = T.axis.reduce(224, k_1_0 * 112 + k_1_1 * 16 + k_1_2)
                                            T.reads(C_rf_global_local[vk_0, v_i], A_local[v_i, vk_0 * 224 + vk_1], B_local[vk_0 * 224 + vk_1])
                                            T.writes(C_rf_global_local[vk_0, v_i])
                                            T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SSSRSRSR"})
                                            C_rf_global_local[vk_0, v_i] = C_rf_global_local[vk_0, v_i] + A_local[v_i, vk_0 * 224 + vk_1] * B_local[vk_0 * 224 + vk_1]
                                for ax0, ax1 in T.grid(1, 2):
                                    with T.block("C_rf_global_local"):
                                        v0 = T.axis.spatial(32, k_0 + ax0)
                                        v1 = T.axis.spatial(28672, i_0 * 512 + i_1 * 32 + i_3 * 2 + ax1)
                                        T.reads(C_rf_global_local[v0, v1])
                                        T.writes(C_rf_global[v0, v1])
                                        C_rf_global[v0, v1] = C_rf_global_local[v0, v1]
            for i in range(28672):
                with T.block("C_init"):
                    v_i = T.axis.spatial(28672, i)
                    T.reads()
                    T.writes(C[v_i])
...
# from tvm import tir
def apply_trace_gptj_mtv_28672_1_7168(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, l3 = sch.get_loops(block=b0)
  v4, v5 = sch.sample_perfect_tile2(loop=l3, n=2, min_n_splits=2, max_n_splits=256, decision=[32, 224])
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[56, 16, 1, 16, 2])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 7, 16])
  ...
  v34 = sch.sample_categorical(candidates=[0, 16, 64, 512], probs=[0.25, 0.25, 0.25, 0.25], decision=0)
