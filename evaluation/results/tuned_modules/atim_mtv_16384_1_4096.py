...
@I.ir_module
class module_atim_mtv_16384_1_4096:
    @T.prim_func
    def main(A: T.Buffer((16384, 4096), "int32"), B: T.Buffer((4096,), "int32"), C: T.Buffer((16384,), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((8, 16384), "int32")
            C_rf_global_local = T.alloc_buffer((8, 16384), "int32", scope="local")
                        ...
                        for i_2 in range(2):
                            for i_3_init, i_4_init in T.grid(2, 1):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(16384, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3_init + i_4_init)
                            ...
                            for k_1_0 in range(16):
                                for ax0, ax1 in T.grid(2, 32):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(16384, i_0 * 64 + i_1 * 4 + i_2 * 2 + ax0)
                                        v1 = T.axis.spatial(4096, k_0 * 512 + k_1_0 * 32 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(32):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(4096, k_0 * 512 + k_1_0 * 32 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_3 in range(2):
                                    for k_1_1, i_4, k_1_2 in T.grid(8, 1, 4):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(16384, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3 + i_4)
                                            vk_0 = T.axis.spatial(8, k_0)
                                            vk_1 = T.axis.reduce(512, k_1_0 * 32 + k_1_1 * 4 + k_1_2)
                                            T.reads(C_rf_global_local[vk_0, v_i], A_local[v_i, vk_0 * 512 + vk_1], B_local[vk_0 * 512 + vk_1])
                                            T.writes(C_rf_global_local[vk_0, v_i])
                                            T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SSSRSRSR"})
                                            C_rf_global_local[vk_0, v_i] = C_rf_global_local[vk_0, v_i] + A_local[v_i, vk_0 * 512 + vk_1] * B_local[vk_0 * 512 + vk_1]
                                    for ax0, ax1 in T.grid(1, 1):
                                        with T.block("C_rf_global_local"):
                                            v0 = T.axis.spatial(8, k_0 + ax0)
                                            v1 = T.axis.spatial(16384, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3 + ax1)
                                            T.reads(C_rf_global_local[v0, v1])
                                            T.writes(C_rf_global[v0, v1])
                                            C_rf_global[v0, v1] = C_rf_global_local[v0, v1]
            for i in range(16384):
                with T.block("C_init"):
                    v_i = T.axis.spatial(16384, i)
                    T.reads()
                    T.writes(C[v_i])
...
# from tvm import tir
def apply_trace_atim_mtv_16384_1_4096(sch: tir.Schedule) -> None:
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[256, 16, 2, 2, 1])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[16, 8, 4])
