...
@I.ir_module
class module_gptj_mmtv_16_128_256:
    @T.prim_func
    def main(A: T.Buffer((16, 128, 256), "int32"), B: T.Buffer((16, 256), "int32"), C: T.Buffer((16, 128), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((16, 8, 128), "int32")
            C_rf_global_local = T.alloc_buffer((16, 8, 128), "int32", scope="local")
                    ...
                    for j_0 in T.thread_binding(2, thread="blockIdx.z", annotations={"bank": T.int64(1)}):
                        for j_1 in T.thread_binding(16, thread="threadIdx.x"):
                            for i_1, i_2, j_2 in T.grid(1, 1, 1):
                                ...
                                for k_1_0, i_3, j_3, k_1_1 in T.grid(1, 1, 2, 2):
                                    for ax0, ax1, ax2 in T.grid(1, 2, 16):
                                        with T.block("A_local"):
                                            v0 = T.axis.spatial(16, i_0 + ax0)
                                            v1 = T.axis.spatial(128, j_0 * 64 + j_1 * 4 + j_3 * 2 + ax1)
                                            v2 = T.axis.spatial(256, k_0 * 32 + k_1_1 * 16 + ax2)
                                            T.reads(A[v0, v1, v2])
                                    ...
                                    for ax0, ax1 in T.grid(1, 16):
                                        with T.block("B_local"):
                                            v0 = T.axis.spatial(16, i_0 + ax0)
                                            v1 = T.axis.spatial(256, k_0 * 32 + k_1_1 * 16 + ax1)
                                            T.reads(B[v0, v1])
                                    ...
                                    for i_4, j_4, k_1_2 in T.grid(1, 2, 16):
                                        with T.block("C_rf_update"):
                                            ...
                                            vk_0 = T.axis.spatial(8, k_0)
                                            vk_1 = T.axis.reduce(32, k_1_0 * 32 + k_1_1 * 16 + k_1_2)
...
# from tvm import tir
def apply_trace_gptj_mmtv_16_128_256(sch: tir.Schedule) -> None:
  ...
  v35, v36, v37 = sch.sample_perfect_tile(loop=l14, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[1, 2, 16])
