...
@I.ir_module
class module_gptj_mmtv_64_64_256:
    @T.prim_func
    def main(A: T.Buffer((64, 64, 256), "int32"), B: T.Buffer((64, 256), "int32"), C: T.Buffer((64, 64), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((64, 4, 64), "int32")
            C_rf_global_local = T.alloc_buffer((64, 4, 64), "int32", scope="local")
            A_local = T.alloc_buffer((64, 64, 256), "int32", scope="local")
            B_local = T.alloc_buffer((64, 256), "int32", scope="local")
                            ...
                            for i_1, i_2, j_2 in T.grid(1, 1, 2):
                                for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 2, 1, 1):
                                    with T.block("C_rf_init"):
                                        v_i = T.axis.spatial(64, i_0 + i_1 + i_2 + i_3_init + i_4_init)
                                        v_j = T.axis.spatial(64, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3_init + j_4_init)
                                ...
                                for k_1_0 in range(4):
                                    for ax0, ax1, ax2 in T.grid(1, 2, 16):
                                        with T.block("A_local"):
                                            v0 = T.axis.spatial(64, i_0 + ax0)
                                            v1 = T.axis.spatial(64, j_1 * 4 + j_2 * 2 + ax1)
                                    ...
                                    for i_3, j_3, k_1_1, i_4, j_4, k_1_2 in T.grid(1, 2, 8, 1, 1, 2):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(64, i_0 + i_1 + i_2 + i_3 + i_4)
                                            v_j = T.axis.spatial(64, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3 + j_4)
                                            vk_0 = T.axis.spatial(4, k_0)
                                            vk_1 = T.axis.reduce(64, k_1_0 * 16 + k_1_1 * 2 + k_1_2)
                                ...
                                for ax0, ax1, ax2 in T.grid(1, 1, 2):
                                    with T.block("C_rf_global_local"):
                                        ...
                                        v2 = T.axis.spatial(64, j_1 * 4 + j_2 * 2 + ax2)
                                        T.reads(C_rf_global_local[v0, v1, v2])
                                        T.writes(C_rf_global[v0, v1, v2])
                                        C_rf_global[v0, v1, v2] = C_rf_global_local[v0, v1, v2]
            for i in T.serial(64, annotations={"pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                for j in range(64):
                    with T.block("C_init"):
...
# from tvm import tir
def apply_trace_gptj_mmtv_64_64_256(sch: tir.Schedule) -> None:
  ...
  v15, v16, v17, v18, v19 = sch.sample_perfect_tile(loop=l11, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[64, 1, 1, 1, 1])
  l20, l21, l22, l23, l24 = sch.split(loop=l11, factors=[v15, v16, v17, v18, v19], preserve_unit_iters=True)
  v25, v26, v27, v28, v29 = sch.sample_perfect_tile(loop=l12, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[1, 16, 2, 2, 1])
  l30, l31, l32, l33, l34 = sch.split(loop=l12, factors=[v25, v26, v27, v28, v29], preserve_unit_iters=True)
  v35, v36, v37 = sch.sample_perfect_tile(loop=l14, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[4, 8, 2])
