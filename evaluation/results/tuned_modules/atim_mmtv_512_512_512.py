...
@I.ir_module
class module_atim_mmtv_512_512_512:
    @T.prim_func
    def main(A: T.Buffer((512, 512, 512), "int32"), B: T.Buffer((512, 512), "int32"), C: T.Buffer((512, 512), "int32")):
        ...
        with T.block("root"):
            ...
            C_local = T.alloc_buffer((512, 512), "int32", scope="local")
            A_local = T.alloc_buffer((512, 512, 512), "int32", scope="local")
            B_local = T.alloc_buffer((512, 512), "int32", scope="local")
            for i_0 in T.thread_binding(512, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                    ...
                    for j_1 in T.thread_binding(16, thread="threadIdx.x"):
                        for i_1, i_2, j_2 in T.grid(1, 1, 2):
                            for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 4, 1, 1):
                                with T.block("C_init"):
                                    v_i = T.axis.spatial(512, i_0 + i_1 + i_2 + i_3_init + i_4_init)
                                    ...
                                    C_local[v_i, v_j] = 0
                            for k_0, i_3, j_3, k_1 in T.grid(4, 1, 4, 1):
                                for ax0, ax1, ax2 in T.grid(1, 1, 128):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(512, i_0 + ax0)
                                        v1 = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 4 + j_3 + ax1)
                                        v2 = T.axis.spatial(512, k_0 * 128 + ax2)
                                        T.reads(A[v0, v1, v2])
                                ...
                                for ax0, ax1 in T.grid(1, 128):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(512, i_0 + ax0)
                                        v1 = T.axis.spatial(512, k_0 * 128 + ax1)
                                        T.reads(B[v0, v1])
                                ...
                                for i_4, j_4, k_2 in T.grid(1, 1, 128):
                                    with T.block("C_update"):
                                        v_i = T.axis.spatial(512, i_0 + i_1 + i_2 + i_3 + i_4)
                                        v_j = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 4 + j_3 + j_4)
                                        v_k = T.axis.reduce(512, k_0 * 128 + k_1 * 128 + k_2)
                            ...
                            for ax0, ax1 in T.grid(1, 4):
                                with T.block("C_local"):
                                    v0 = T.axis.spatial(512, i_0 + ax0)
                                    v1 = T.axis.spatial(512, j_0 * 128 + j_1 * 8 + j_2 * 4 + ax1)
                                    T.reads(C_local[v0, v1])
                                    T.writes(C[v0, v1])
                                    C[v0, v1] = C_local[v0, v1]
# from tvm import tir
def apply_trace_atim_mmtv_512_512_512(sch: tir.Schedule) -> None:
  ...
  v25, v26, v27 = sch.sample_perfect_tile(loop=l4, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[4, 1, 128])
