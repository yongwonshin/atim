...
@I.ir_module
class module_atim_mtv_7168_1_7168:
    @T.prim_func
    def main(A: T.Buffer((7168, 7168), "int32"), B: T.Buffer((7168,), "int32"), C: T.Buffer((7168,), "int32")):
        ...
        with T.block("root"):
            ...
            for k_0 in T.thread_binding(16, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(0)}):
                for i_0 in T.thread_binding(112, thread="blockIdx.y", annotations={"bank": T.int64(1)}):
                    for i_1 in T.thread_binding(16, thread="threadIdx.x"):
                        for i_2 in range(2):
                            for i_3_init, i_4_init in T.grid(2, 1):
                                with T.block("C_rf_init"):
                                    v_i = T.axis.spatial(7168, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3_init + i_4_init)
                            ...
                            for k_1_0, i_3, k_1_1 in T.grid(8, 2, 1):
                                for ax0, ax1 in T.grid(1, 56):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(7168, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3 + ax0)
                                        v1 = T.axis.spatial(7168, k_0 * 448 + k_1_0 * 56 + ax1)
                                        T.reads(A[v0, v1])
                                ...
                                for ax0 in range(56):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(7168, k_0 * 448 + k_1_0 * 56 + ax0)
                                        T.reads(B[v0])
                                        T.writes(B_local[v0])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0] = B[v0]
                                for i_4, k_1_2 in T.grid(1, 56):
                                    with T.block("C_rf_update"):
                                        v_i = T.axis.spatial(7168, i_0 * 64 + i_1 * 4 + i_2 * 2 + i_3 + i_4)
                                        vk_0 = T.axis.spatial(16, k_0)
                                        vk_1 = T.axis.reduce(448, k_1_0 * 56 + k_1_1 * 56 + k_1_2)
                                        T.reads(C_rf_global_local[vk_0, v_i], A_local[v_i, vk_0 * 448 + vk_1], B_local[vk_0 * 448 + vk_1])
                                        T.writes(C_rf_global_local[vk_0, v_i])
                                        T.block_attr({"meta_schedule.meta_schedule_rfactor_producer_block": T.int64(1), "meta_schedule.tiling_structure": "SSSRSRSR"})
                                        C_rf_global_local[vk_0, v_i] = C_rf_global_local[vk_0, v_i] + A_local[v_i, vk_0 * 448 + vk_1] * B_local[vk_0 * 448 + vk_1]
                            for ax0, ax1 in T.grid(1, 2):
                                with T.block("C_rf_global_local"):
                                    v0 = T.axis.spatial(16, k_0 + ax0)
                                    v1 = T.axis.spatial(7168, i_0 * 64 + i_1 * 4 + i_2 * 2 + ax1)
                                    T.reads(C_rf_global_local[v0, v1])
                                    T.writes(C_rf_global[v0, v1])
                                    C_rf_global[v0, v1] = C_rf_global_local[v0, v1]
            for i in T.serial(7168, annotations={"pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(0)}):
                with T.block("C_init"):
                    v_i = T.axis.spatial(7168, i)
                    T.reads()
                    T.writes(C[v_i])
...
# from tvm import tir
def apply_trace_atim_mtv_7168_1_7168(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, l3 = sch.get_loops(block=b0)
  v4, v5 = sch.sample_perfect_tile2(loop=l3, n=2, min_n_splits=2, max_n_splits=256, decision=[16, 448])
  ...
  v13, v14, v15, v16, v17 = sch.sample_perfect_tile(loop=l10, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[112, 16, 2, 2, 1])
  l18, l19, l20, l21, l22 = sch.split(loop=l10, factors=[v13, v14, v15, v16, v17], preserve_unit_iters=True)
  v23, v24, v25 = sch.sample_perfect_tile(loop=l12, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[8, 1, 56])
  ...
  v34 = sch.sample_categorical(candidates=[0, 16, 64, 512], probs=[0.25, 0.25, 0.25, 0.25], decision=1)
