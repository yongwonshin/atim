...
@I.ir_module
class module_atim_mmtv_112_64_256:
    @T.prim_func
    def main(A: T.Buffer((112, 64, 256), "int32"), B: T.Buffer((112, 256), "int32"), C: T.Buffer((112, 64), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((112, 4, 64), "int32")
            C_rf_global_local = T.alloc_buffer((112, 4, 64), "int32", scope="local")
            A_local = T.alloc_buffer((112, 64, 256), "int32", scope="local")
            B_local = T.alloc_buffer((112, 256), "int32", scope="local")
            for k_0 in T.thread_binding(4, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                for i_0 in T.thread_binding(112, thread="blockIdx.y", annotations={"bank": T.int64(1)}):
                            ...
                            for i_1, i_2, j_2 in T.grid(1, 1, 1):
                                for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 2, 1, 1):
                                    with T.block("C_rf_init"):
                                        ...
                                        v_j = T.axis.spatial(64, j_0 * 32 + j_1 * 2 + j_2 * 2 + j_3_init + j_4_init)
                                ...
                                for k_1_0 in range(4):
                                    for ax0, ax1, ax2 in T.grid(1, 2, 16):
                                        with T.block("A_local"):
                                            ...
                                            v2 = T.axis.spatial(256, k_0 * 64 + k_1_0 * 16 + ax2)
                                            T.reads(A[v0, v1, v2])
                                    ...
                                    for ax0, ax1 in T.grid(1, 16):
                                        with T.block("B_local"):
                                            v0 = T.axis.spatial(112, i_0 + ax0)
                                            v1 = T.axis.spatial(256, k_0 * 64 + k_1_0 * 16 + ax1)
                                            T.reads(B[v0, v1])
                                            T.writes(B_local[v0, v1])
                                            T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                            B_local[v0, v1] = B[v0, v1]
                                    for i_3, j_3, k_1_1, i_4, j_4, k_1_2 in T.grid(1, 2, 16, 1, 1, 1):
                                        with T.block("C_rf_update"):
                                            v_i = T.axis.spatial(112, i_0 + i_1 + i_2 + i_3 + i_4)
                                            v_j = T.axis.spatial(64, j_0 * 32 + j_1 * 2 + j_2 * 2 + j_3 + j_4)
                                            vk_0 = T.axis.spatial(4, k_0)
                                            vk_1 = T.axis.reduce(64, k_1_0 * 16 + k_1_1 + k_1_2)
            ...
            for i in T.serial(112, annotations={"pragma_auto_unroll_max_step": T.int64(512), "pragma_unroll_explicit": T.int64(0)}):
                for j in range(64):
                    with T.block("C_init"):
...
# from tvm import tir
def apply_trace_atim_mmtv_112_64_256(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, l3, l4 = sch.get_loops(block=b0)
  v5, v6 = sch.sample_perfect_tile2(loop=l4, n=2, min_n_splits=2, max_n_splits=256, decision=[4, 64])
  ...
  v15, v16, v17, v18, v19 = sch.sample_perfect_tile(loop=l11, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[112, 1, 1, 1, 1])
  l20, l21, l22, l23, l24 = sch.split(loop=l11, factors=[v15, v16, v17, v18, v19], preserve_unit_iters=True)
  v25, v26, v27, v28, v29 = sch.sample_perfect_tile(loop=l12, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 16, 1, 2, 1])
  l30, l31, l32, l33, l34 = sch.split(loop=l12, factors=[v25, v26, v27, v28, v29], preserve_unit_iters=True)
  v35, v36, v37 = sch.sample_perfect_tile(loop=l14, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[4, 16, 1])
  ...
  v46 = sch.sample_categorical(candidates=[0, 16, 64, 512], probs=[0.25, 0.25, 0.25, 0.25], decision=3)
  ...
  sch.annotate(block_or_loop=l54, ann_key="pragma_auto_unroll_max_step", ann_val=512)
  sch.annotate(block_or_loop=l54, ann_key="pragma_unroll_explicit", ann_val=0)
  ...
  sch.annotate(block_or_loop=l89, ann_key="pragma_unroll_explicit", ann_val=0)
