...
@I.ir_module
class module_atim_mmtv_28_256_256:
    @T.prim_func
    def main(A: T.Buffer((28, 256, 256), "int32"), B: T.Buffer((28, 256), "int32"), C: T.Buffer((28, 256), "int32")):
        ...
        with T.block("root"):
            ...
            C_rf_global = T.alloc_buffer((28, 8, 256), "int32")
            C_rf_global_local = T.alloc_buffer((28, 8, 256), "int32", scope="local")
            A_local = T.alloc_buffer((28, 256, 256), "int32", scope="local")
            B_local = T.alloc_buffer((28, 256), "int32", scope="local")
            for k_0 in T.thread_binding(8, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(0)}):
                            ...
                            for i_1, i_2, j_2 in T.grid(1, 1, 1):
                                for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 8, 1, 1):
                                    with T.block("C_rf_init"):
                                        ...
                                        v_j = T.axis.spatial(256, j_0 * 128 + j_1 * 8 + j_2 * 8 + j_3_init + j_4_init)
                                ...
                                for k_1_0 in range(1):
                                    for ax0, ax1, ax2 in T.grid(1, 8, 32):
                                        with T.block("A_local"):
                                            ...
                                            v1 = T.axis.spatial(256, j_0 * 128 + j_1 * 8 + ax1)
                                    ...
                                    for i_3, j_3 in T.grid(1, 8):
                                        for k_1_1, i_4, j_4, k_1_2 in T.grid(8, 1, 1, 4):
                                            with T.block("C_rf_update"):
                                                v_i = T.axis.spatial(28, i_0 + i_1 + i_2 + i_3 + i_4)
                                                v_j = T.axis.spatial(256, j_0 * 128 + j_1 * 8 + j_2 * 8 + j_3 + j_4)
                                                vk_0 = T.axis.spatial(8, k_0)
                                                vk_1 = T.axis.reduce(32, k_1_0 * 32 + k_1_1 * 4 + k_1_2)
                                        ...
                                        for ax0, ax1, ax2 in T.grid(1, 1, 1):
                                            with T.block("C_rf_global_local"):
                                                ...
                                                v2 = T.axis.spatial(256, j_0 * 128 + j_1 * 8 + j_3 + ax2)
                                                T.reads(C_rf_global_local[v0, v1, v2])
                                                T.writes(C_rf_global[v0, v1, v2])
                                                C_rf_global[v0, v1, v2] = C_rf_global_local[v0, v1, v2]
            for i in T.serial(28, annotations={"pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(0)}):
                for j in range(256):
                    with T.block("C_init"):
...
# from tvm import tir
def apply_trace_atim_mmtv_28_256_256(sch: tir.Schedule) -> None:
  ...
  v25, v26, v27, v28, v29 = sch.sample_perfect_tile(loop=l12, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 16, 1, 8, 1])
  l30, l31, l32, l33, l34 = sch.split(loop=l12, factors=[v25, v26, v27, v28, v29], preserve_unit_iters=True)
  v35, v36, v37 = sch.sample_perfect_tile(loop=l14, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[1, 8, 4])
  ...
  v46 = sch.sample_categorical(candidates=[0, 16, 64, 512], probs=[0.25, 0.25, 0.25, 0.25], decision=1)
