...
@I.ir_module
class module_atim_mmtv_256_256_256:
    @T.prim_func
    def main(A: T.Buffer((256, 256, 256), "int32"), B: T.Buffer((256, 256), "int32"), C: T.Buffer((256, 256), "int32")):
        ...
        with T.block("root"):
            ...
            C_local = T.alloc_buffer((256, 256), "int32", scope="local")
                        ...
                        for i_1, i_2, j_2 in T.grid(1, 1, 1):
                            for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 4, 1, 1):
                                with T.block("C_init"):
                                    ...
                                    v_j = T.axis.spatial(256, j_0 * 32 + j_1 * 4 + j_2 * 4 + j_3_init + j_4_init)
                                    T.reads()
                            ...
                            for k_0 in range(8):
                                for ax0, ax1, ax2 in T.grid(1, 4, 32):
                                    with T.block("A_local"):
                                        ...
                                        v1 = T.axis.spatial(256, j_0 * 32 + j_1 * 4 + ax1)
                                ...
                                for i_3, j_3, k_1, i_4, j_4, k_2 in T.grid(1, 4, 8, 1, 1, 4):
                                    with T.block("C_update"):
                                        v_i = T.axis.spatial(256, i_0 + i_1 + i_2 + i_3 + i_4)
                                        v_j = T.axis.spatial(256, j_0 * 32 + j_1 * 4 + j_2 * 4 + j_3 + j_4)
                                        v_k = T.axis.reduce(256, k_0 * 32 + k_1 * 4 + k_2)
...
# from tvm import tir
def apply_trace_atim_mmtv_256_256_256(sch: tir.Schedule) -> None:
  ...
  v15, v16, v17, v18, v19 = sch.sample_perfect_tile(loop=l3, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[8, 8, 1, 4, 1])
  l20, l21, l22, l23, l24 = sch.split(loop=l3, factors=[v15, v16, v17, v18, v19], preserve_unit_iters=True)
  v25, v26, v27 = sch.sample_perfect_tile(loop=l4, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[8, 8, 4])
