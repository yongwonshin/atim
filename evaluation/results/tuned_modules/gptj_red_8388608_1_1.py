...
@I.ir_module
class module_gptj_red_8388608_1_1:
    @T.prim_func
        ...
        with T.block("root"):
            ...
            for i_0 in T.thread_binding(512, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(64), "pragma_unroll_explicit": T.int64(0)}):
                for i_1_0 in T.thread_binding(16, thread="threadIdx.x"):
                    ...
                    for i_1_1_0 in range(16):
                        for ax0 in range(64):
                            with T.block("A_local"):
                                v0 = T.axis.spatial(8388608, i_0 * 16384 + i_1_0 * 1024 + i_1_1_0 * 64 + ax0)
                                T.reads(A[v0])
                                T.writes(A_local[v0])
                                A_local[v0] = A[v0]
                        for i_1_1_1 in range(32):
                            for i_1_1_2 in range(2):
                                with T.block("C_rf_rf_update"):
                                    vi_0, vi_1_0 = T.axis.remap("SS", [i_0, i_1_0])
                                    vi_1_1 = T.axis.reduce(1024, i_1_1_0 * 64 + i_1_1_1 * 2 + i_1_1_2)
...
# from tvm import tir
def apply_trace_gptj_red_8388608_1_1(sch: tir.Schedule) -> None:
  b0 = sch.get_block(name="C", func_name="main")
  b1 = sch.get_block(name="root", func_name="main")
  l2, = sch.get_loops(block=b0)
  v3, v4 = sch.sample_perfect_tile2(loop=l2, n=2, min_n_splits=2, max_n_splits=2048, decision=[512, 16384])
  ...
  v11, v12 = sch.sample_perfect_tile2(loop=l10, n=2, min_n_splits=2, max_n_splits=24, decision=[16, 1024])
  ...
  v23, v24, v25 = sch.sample_perfect_tile(loop=l22, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[16, 32, 2])
  ...
  l48, l49, l50, l51 = sch.get_loops(block=b41)
  ...
  l52, l53, l54, l55, l56 = sch.get_loops(block=b42)
  sch.annotate(block_or_loop=l52, ann_key="pragma_auto_unroll_max_step", ann_val=64)
  sch.annotate(block_or_loop=l52, ann_key="pragma_unroll_explicit", ann_val=0)
  l57, l58, l59, l60, l61, l62, l63 = sch.get_loops(block=b43)
  sch.annotate(block_or_loop=l57, ann_key="pragma_auto_unroll_max_step", ann_val=64)
  sch.annotate(block_or_loop=l57, ann_key="pragma_unroll_explicit", ann_val=0)
  ...
  sch.annotate(block_or_loop=l66, ann_key="pragma_auto_unroll_max_step", ann_val=64)
