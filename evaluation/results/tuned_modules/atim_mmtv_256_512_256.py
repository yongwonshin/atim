...
@I.ir_module
class module_atim_mmtv_256_512_256:
    @T.prim_func
                        ...
                        for i_1, i_2, j_2 in T.grid(1, 1, 2):
                            for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 2, 1, 1):
                                with T.block("C_init"):
                                    ...
                                    v_j = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3_init + j_4_init)
                                    T.reads()
                                    T.writes(C_local[v_i, v_j])
                                    T.block_attr({"meta_schedule.tiling_structure": "SSSRSRSR"})
                                    C_local[v_i, v_j] = 0
                            for k_0, i_3, j_3, k_1 in T.grid(2, 1, 2, 2):
                                for ax0, ax1, ax2 in T.grid(1, 1, 64):
                                    with T.block("A_local"):
                                        v0 = T.axis.spatial(256, i_0 + ax0)
                                        v1 = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3 + ax1)
                                        v2 = T.axis.spatial(256, k_0 * 128 + k_1 * 64 + ax2)
                                        T.reads(A[v0, v1, v2])
                                ...
                                for ax0, ax1 in T.grid(1, 64):
                                    with T.block("B_local"):
                                        v0 = T.axis.spatial(256, i_0 + ax0)
                                        v1 = T.axis.spatial(256, k_0 * 128 + k_1 * 64 + ax1)
                                        T.reads(B[v0, v1])
                                        T.writes(B_local[v0, v1])
                                        T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                        B_local[v0, v1] = B[v0, v1]
                                for i_4, j_4, k_2 in T.grid(1, 1, 64):
                                    with T.block("C_update"):
                                        v_i = T.axis.spatial(256, i_0 + i_1 + i_2 + i_3 + i_4)
                                        v_j = T.axis.spatial(512, j_0 * 64 + j_1 * 4 + j_2 * 2 + j_3 + j_4)
                                        v_k = T.axis.reduce(256, k_0 * 128 + k_1 * 64 + k_2)
...
# from tvm import tir
def apply_trace_atim_mmtv_256_512_256(sch: tir.Schedule) -> None:
  ...
  v15, v16, v17, v18, v19 = sch.sample_perfect_tile(loop=l3, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[8, 16, 2, 2, 1])
  l20, l21, l22, l23, l24 = sch.split(loop=l3, factors=[v15, v16, v17, v18, v19], preserve_unit_iters=True)
  v25, v26, v27 = sch.sample_perfect_tile(loop=l4, n=3, max_innermost_factor=256, min_innermost_factor=1, decision=[2, 2, 64])
