...
@I.ir_module
class module_gptj_va_1048576_1_1:
    @T.prim_func
    def main(A: T.Buffer((1048576,), "int32"), B: T.Buffer((1048576,), "int32"), C: T.Buffer((1048576,), "int32")):
        ...
        with T.block("root"):
            ...
            for i_0 in T.thread_binding(1024, thread="blockIdx.x", annotations={"bank": T.int64(1), "pragma_auto_unroll_max_step": T.int64(16), "pragma_unroll_explicit": T.int64(0)}):
                for i_1 in T.thread_binding(8, thread="threadIdx.x"):
                    for i_2 in range(4):
                        for ax0 in range(32):
                            with T.block("A_local"):
                                v0 = T.axis.spatial(1048576, i_0 * 1024 + i_1 * 128 + i_2 * 32 + ax0)
                                T.reads(A[v0])
                                T.writes(A_local[v0])
                        ...
                        for ax0 in range(32):
                            with T.block("B_local"):
                                ...
                                T.block_attr({"meta_schedule.cooperative_fetch": 1})
                                B_local[v0] = B[v0]
                        for i_3, i_4 in T.grid(8, 4):
                            with T.block("C"):
                                v_i = T.axis.spatial(1048576, i_0 * 1024 + i_1 * 128 + i_2 * 32 + i_3 * 4 + i_4)
                        ...
                        for ax0 in range(32):
                            with T.block("C_local"):
                                ...
                                C[v0] = C_local[v0]
# from tvm import tir
def apply_trace_gptj_va_1048576_1_1(sch: tir.Schedule) -> None:
  ...
  v3, v4, v5, v6, v7 = sch.sample_perfect_tile(loop=l2, n=5, max_innermost_factor=256, min_innermost_factor=1, decision=[1024, 8, 4, 8, 4])
