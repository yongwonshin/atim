...
static void vector_addition_host(T* C, T* A, T* B,
  unsigned int nr_elements, int alpha, int beta) {
...
int main(int argc, char** argv) {
  ...
  for (int rep = 0; rep < 1; rep++) {
    ...
    vector_addition_host(C, A, B, input_size, alpha, beta);
    ...
    DPU_ASSERT(dpu_broadcast_to(dpu_set, "coeff", 0, coeff, 8, DPU_XFER_DEFAULT));
