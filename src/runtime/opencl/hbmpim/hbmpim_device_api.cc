...
int HBMPIMWorkspace::ExecuteGemmBiasAct(void* output, void* pim_data,
  ...
  //   std::cerr << std::endl;
  // }
  pim_sim_.preload_data_with_addr(pim_data_addr - pim_base_addr, input_data,
...
bool HBMPIMWorkspace::IsOpenCLDevice(Device dev) {
  return dev.device_type == static_cast<DLDeviceType>(kDLHBMPIM) ||
         dev.device_type == static_cast<DLDeviceType>(kDLOpenCL);
}

bool HBMPIMWorkspace::IsHBMPIMDevice(Device dev) {
  return dev.device_type == static_cast<DLDeviceType>(kDLHBMPIM);
}
...
void HBMPIMWorkspace::CopyDataFromTo(DLTensor* from, DLTensor* to, TVMStreamHandle stream) {
  ...
  if (IsHBMPIMDevice(from->device) && to->device.device_type == kDLCPU) {
  ...
  } else if (from->device.device_type == kDLCPU && IsHBMPIMDevice(to->device)) {
