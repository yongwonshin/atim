...
std::vector<int64_t> SamplePerfectTile(support::LinearCongruentialEngine::TRandState* rand_state,
  ...
  if (max_innermost_factor == -1 && min_innermost_factor == -1) {
...
// TODO[ywshin]: SamplePerfectTile2
std::vector<int64_t> SamplePerfectTile2(
    support::LinearCongruentialEngine::TRandState* rand_state,  //
    const tir::StmtSRef& loop_sref, int32_t n_splits, int32_t min_n_splits, int32_t max_n_splits,
...
struct SamplePerfectTile2Traits : public UnpackedInstTraits<SamplePerfectTile2Traits> {
 ...
 private:
  ...
  static Array<ExprRV> UnpackedApplyToSchedule(Schedule sch, LoopRV loop_rv, Integer n,
                                               Integer min_n_splits, Integer max_n_splits,
                                               Optional<Array<Integer>> decision) {
    return sch->SamplePerfectTile2(loop_rv, n->value, min_n_splits->value, max_n_splits->value,
                                   decision);
  }
  ...
  static String UnpackedAsPython(Array<String> outputs, String loop_rv, Integer n,
                                 Integer min_n_splits, Integer max_n_splits,
                                 Optional<Array<Integer>> decision) {
    PythonAPICall py("sample_perfect_tile2");
    py.Input("loop", loop_rv);
    py.Input("n", n->value);
    py.Input("min_n_splits", min_n_splits->value);
    py.Input("max_n_splits", max_n_splits->value);
    py.Decision(decision);
    py.OutputList(outputs);
    return py.Str();
  }
